...
DocSpanInfo = namedtuple(
    'DocSpanInfo', (
        ...
        'span2span_target',
        # 'pred_span_mention_range_list'
        # 'pred_mention_drange_list'
        # 'pred_mention_type_list',
    ),
)
DocSpanInfo.__new__.__defaults__ = (None, )

def get_ner_eval_on_doc(token_type, doc_fea):
    pred_token_type_mat = token_type.tolist()
    pred_span_token_tup_list, pred_span_dranges_list = extract_doc_valid_span_info(pred_token_type_mat, doc_fea)
    pred_span_mention_range_list, pred_mention_drange_list, pred_mention_type_list =  get_span_mention_info(
        pred_span_dranges_list, pred_token_type_mat
    )
    pred_res = {}
    for i, pred_span_metion_range in enumerate(pred_span_mention_range_list):
        token_ids = pred_span_token_tup_list[i]
        drange_list = pred_mention_drange_list[pred_span_metion_range[0]: pred_span_metion_range[1]]
        label = pred_mention_type_list[pred_span_metion_range[0]: pred_span_metion_range[1]]
        pred_res[token_ids] = dict(zip(drange_list, label))
    return pred_res
...
def get_doc_span_info_list(doc_token_types_list, doc_fea_list, use_gold_span=False):
    ...
    for doc_token_types, doc_fea in zip(doc_token_types_list, doc_fea_list):
        ...
        if use_gold_span:
            ...
            pred_span_token_tup_list, pred_span_dranges_list = extract_doc_valid_span_info(doc_token_type_mat, doc_fea)
            if len(pred_span_token_tup_list) == 0:
                ...
                span_token_tup_list = pred_span_token_tup_list
                span_dranges_list = pred_span_dranges_list
        ...
        for event_idx, event_pred in enumerate(event_pred_list):
            ...
            for field_idx in range(num_fields):
                ...
                for prev_path in last_field_paths:  # traverse all previous decoding paths
                    ...
                    if sum(span_pred_list) > 1:
                        span_pred_list =  [0 for _ in span_pred_list]
                        span_pred_list[0] = 1
...
class DCFEEModel(nn.Module):
    ...
    def get_local_context_info(self, doc_batch_dict, train_flag=False, use_gold_span=False):
        ...
        ner_token_masks = []
        ner_token_labels = [] if need_label_flag else None
        ner_cut_word_labels = [] if doc_batch_dict['doc_cut_word_labels'][0] is not None else None
        for batch_idx, valid_sent_num in enumerate(valid_sent_num_list):
            idx_start = ner_batch_idx_start_list[-1]
            idx_end = idx_start + valid_sent_num
            ner_batch_idx_start_list.append(idx_end)
            ...
            if ner_cut_word_labels is not None:
                ner_cut_word_labels.append(doc_batch_dict['doc_cut_word_labels'][batch_idx])
            if need_label_flag:
                ner_token_labels.append(doc_token_labels_list[batch_idx])
        ...
        if need_label_flag:
            ner_token_labels = torch.cat(ner_token_labels, dim=0)
        ner_cut_word_labels = torch.cat(ner_cut_word_labels, dim=0)

        if not self.config.doc_ner:
            # get ner output
            ner_token_emb, ner_loss, ner_token_preds = self.ner_model(
                ner_token_ids, ner_token_masks, label_ids=ner_token_labels,
                train_flag=train_flag, decode_flag=not use_gold_span,
            ...
            ner_token_emb, ner_loss, ner_token_preds, ner_cut_word_preds = self.ner_model(
                ner_token_ids, ner_token_masks, ner_cut_word_labels, ner_batch_idx_start_list, batch_size,
                label_ids=ner_token_labels, train_flag=train_flag, decode_flag=not use_gold_span
            )
        ...
        for batch_idx in range(batch_size):
            ...
            _ner_other_eval_info = {}
            if ner_cut_word_labels is not None and ner_cut_word_preds is not None:
                _ner_other_eval_info['cut_word'] = {
                    'pred': ner_cut_word_preds[idx_start:idx_end].cpu().detach().numpy(),
                    'gt': ner_cut_word_labels[idx_start:idx_end].cpu().detach().numpy()
                }
                ner_other_eval_info.append(_ner_other_eval_info)

        return doc_token_emb_list, doc_token_masks_list, doc_token_types_list, doc_sent_emb_list, doc_sent_loss_list, ner_other_eval_info
    ...
    def forward(self, doc_batch_dict, doc_features,
        ...
        # get doc token-level local context
        if self.config.doc_ner:
            doc_token_emb_list, doc_token_masks_list, doc_token_types_list, doc_sent_emb_list, doc_sent_loss_list, ner_other_eval_info = \
                self.get_local_context_info(
                    doc_batch_dict, train_flag=train_flag, use_gold_span=use_gold_span,
                )
        else: 
        ...
        if train_flag:
            ...
            assert event_idx2entity_idx2field_idx is not None
            for batch_idx, ex_idx in enumerate(ex_idx_list):
                eval_res = list(self.heuristic_decode_on_doc(
                    doc_sent_emb_list[batch_idx],
                    doc_fea_list[batch_idx],
                    doc_span_info_list[batch_idx],
                    event_idx2entity_idx2field_idx,
                ...
                eval_res.append(ner_other_eval_info) #让它带出来

...
from .dee_model import DocSpanInfo, get_ner_eval_on_doc
...
def get_doc_span_info_list(doc_token_types_list, doc_fea_list, use_gold_span=False):
    ...
    for doc_token_types, doc_fea in zip(doc_token_types_list, doc_fea_list):
        ...
        event_dag_info, span2span_target, _, missed_sent_idx_list = doc_fea.generate_graph_info_for(span_token_tup_list, return_miss=True)
        ...
        doc_span_info = DocSpanInfo(
            ...
            event_dag_info, missed_sent_idx_list, span2span_target
        )
...
class GraphModel(nn.Module):
    ...
    def __init__(self, config, event_type_fields_pairs, ner_model=None):
        ...
        self.span2span_att = AttentionVal(config.hidden_size)
    ...
    def get_field_mle_loss_list(self, doc_sent_context, batch_span_context, event_idx, field_idx2pre_path2cur_span_idx_set,
                                span2span_score, memory_mechanism=None):
        field_mle_loss_list = []
        num_fields = self.event_tables[event_idx].num_fields
        num_spans = batch_span_context.size(0)

        if memory_mechanism is None:
            prev_path2prev_decode_context = {
                (): doc_sent_context
            }
        elif memory_mechanism == 'MEAN':
            prev_path2prev_decode_context = {
                (): (batch_span_context, None)
            }
        ...
        for field_idx in range(num_fields):
            ...
            for prev_path, cur_span_idx_set in prev_path2cur_span_idx_set.items():
                ...
                if memory_mechanism == 'MEAN':
                    prev_decode_span2span_context = prev_decode_context[0] / (field_idx + 1) # mean
                    prev_decode_last_span_context = prev_decode_context[1]
                    if prev_decode_last_span_context is not None:
                        prev_decode_context = torch.cat([
                            doc_sent_context, prev_decode_span2span_context, prev_decode_last_span_context
                        ], dim=0)
                    else:
                        prev_decode_context = torch.cat([
                            doc_sent_context, prev_decode_span2span_context
                        ], dim=0)
                ...
                for span_idx in cur_span_idx_set:
                    ...
                    if memory_mechanism is None:
                        ...
                        if span_idx is None:
                            cur_decode_span2span_context = prev_decode_span2span_context + batch_span_context
                            cur_last_span_context = self.event_tables[event_idx].field_queries[field_idx]
                            if prev_decode_last_span_context is not None:
                                cur_decode_last_span_context = torch.cat([prev_decode_last_span_context, cur_last_span_context], dim=0)
                            else:
                                cur_decode_last_span_context = cur_last_span_context

                            prev_path2prev_decode_context[cur_path] = (
                                cur_decode_span2span_context, cur_decode_last_span_context
                            )
                        else:
                            cur_decode_span2span_context = prev_decode_span2span_context + \
                                span2span_score[span_idx].view(-1, 1) * batch_span_context
                            cur_last_span_context = batch_cand_emb[span_idx].unsqueeze(0)
                            if prev_decode_last_span_context is not None:
                                cur_decode_last_span_context = torch.cat([prev_decode_last_span_context, cur_last_span_context], dim=0)
                            else:
                                cur_decode_last_span_context = cur_last_span_context
                            ...
                            # cur_span_context = prev_decode_context[doc_sent_context.shape[0]:]
                            # cur_span_context += span2span_score[span_idx].view(-1, 1) * batch_span_context
                            # prev_path2prev_decode_context[cur_path] = cur_span_context
        return field_mle_loss_list

    def get_span2span_loss(self, batch_span_context, span2span_target, train_flag):
        device = batch_span_context.device
        span2span_score = self.span2span_att(batch_span_context)

        span2span_loss = None
        if train_flag:
            span2span_target = torch.tensor(span2span_target, dtype=torch.float, device=device)
            span2span_loss = torch.nn.functional.mse_loss(span2span_score, span2span_target, reduction='sum')
        return span2span_loss, span2span_score, span2span_target

    def get_loss_on_doc(self, doc_token_emb, doc_sent_emb, doc_fea, doc_span_info, use_gold_span, memory_mechanism=None):
        record_info = {}
        ...
        # span2span loss
        span2span_loss, span2span_score, gold_span2span_score = \
            self.get_span2span_loss(batch_span_context, doc_span_info.span2span_target, train_flag=True)
        if use_gold_span:
            span2span_score = gold_span2span_score
        # span2span_score = gold_span2span_score

        record_info['span2span_loss'] = span2span_loss.detach().cpu().numpy()
        ...
        for event_idx, event_label in enumerate(doc_fea.event_type_labels):
            if event_label == 0:
                ...
                if memory_mechanism is None:
                    ...
                    prev_decode_context = torch.cat([doc_sent_context, batch_span_context], dim=0)
                ...
                field_loss_list = self.get_field_mle_loss_list(
                    ...
                    span2span_score, memory_mechanism=memory_mechanism
                )
                all_field_loss_list += field_loss_list


        total_event_loss = event_cls_loss + sum(all_field_loss_list) + 0.3 * span2span_loss
        record_info['total_loss'] = total_event_loss.detach().cpu().numpy()
        return total_event_loss, record_info
    ...
    def get_eval_on_doc(self, doc_token_emb, doc_sent_emb, doc_fea, doc_span_info, use_gold_span, memory_mechanism):
        span_context_list, doc_sent_context = self.get_doc_span_sent_context(
            doc_token_emb, doc_sent_emb, doc_fea, doc_span_info
        )
        if len(span_context_list) == 0:
        ...
        _, span2span_score, gold_span2span_score = \
            self.get_span2span_loss(batch_span_context, doc_span_info.span2span_target, train_flag=False)
        # if use_gold_span:
        #     span2span_score = gold_span2span_score
            ...
            for field_idx in range(num_fields):
                ...
                for prev_path in last_field_paths:  # traverse all previous decoding paths
                    ...
                    if memory_mechanism == 'MEAN':
                        ...
                        # prev_decode_context = prev_decode_context / (field_idx + 1) # mean
                        # prev_decode_context = torch.cat([doc_sent_context, prev_decode_context], dim=0)
                    ...
                    for span_idx in cur_span_idx_list:
                        ...
                        if memory_mechanism is None:
                            ...
                            # if span_idx is None:
                            #     cur_span_context = prev_decode_context[doc_sent_context.shape[0]:]
                            #     # cur_span_context += batch_span_context
                            #     prev_path2prev_decode_context[cur_path] = cur_span_context
                            # else:
                            #     cur_span_context = prev_decode_context[doc_sent_context.shape[0]:]
                            #     cur_span_context += span2span_score[span_idx].view(-1, 1) * batch_span_context
                ...
                if len(cur_paths) > 100:
                    cur_paths = cur_paths[:100]
        ...
        return doc_fea.ex_idx, event_pred_list, event_idx2obj_idx2field_idx2token_tup, \
            doc_span_info, event_idx2event_decode_paths, span2span_score.detach().cpu().numpy()
    ...
    def get_local_context_info(self, doc_batch_dict, train_flag=False, use_gold_span=False, sent_batch_size=None):
    ...
    def forward(self, doc_batch_dict, doc_features,
                ...
                event_idx2entity_idx2field_idx=None, heuristic_type=None, sent_batch_size=2):
        ...
        doc_token_emb_list, doc_token_masks_list, doc_token_types_list, doc_sent_emb_list, doc_sent_loss_list = \
            self.get_local_context_info(
                doc_batch_dict, train_flag=train_flag, use_gold_span=use_gold_span, sent_batch_size=sent_batch_size
            )
        ...
        if train_flag:
            ...
            for batch_idx, ex_idx in enumerate(ex_idx_list):
                ner_results.append(
                    get_ner_eval_on_doc(doc_token_types_list[batch_idx], doc_fea_list[batch_idx])
                )
            for batch_idx, ex_idx in enumerate(ex_idx_list):
                eval_results.append(
            ...
            return eval_results, ner_results

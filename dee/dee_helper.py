...
class DEEExample(object):
    def __init__(self, annguid, detail_align_dict, only_inference=False):
        ...
        if only_inference:
            ...
            self.cut_word_label = detail_align_dict.get('cut_word_label', None)
...
class DEEExampleLoader(object):
    def __init__(self, rearrange_sent_flag, max_sent_len, load_ex_num=None):
        self.rearrange_sent_flag = rearrange_sent_flag
        self.max_sent_len = max_sent_len
        self.load_ex_num = load_ex_num
    ...
    def __call__(self, dataset_json_path):
        ...
        if self.load_ex_num is not None:
            annguid_aligninfo_list = annguid_aligninfo_list[:self.load_ex_num]
...
class DEEFeature(object):
    def __init__(self, guid, ex_idx, doc_token_id_mat, doc_token_mask_mat, doc_token_label_mat, doc_cut_word_label_mat,
                 span_token_ids_list, span_dranges_list, span_label_list, event_type_labels, event_arg_idxs_objs_list,
                 valid_sent_num=None):
        ...
        if len(doc_cut_word_label_mat) > 0 and doc_cut_word_label_mat[0] is not None:
            self.doc_cut_word_labels = torch.tensor(doc_cut_word_label_mat, dtype=torch.long)
        else:
            self.doc_cut_word_labels = None
        ...
        self.span_label_list = span_label_list
        ...
        self.span2span_target = self.build_span2span_target(span_token_ids_list, event_arg_idxs_objs_list)


    def build_span2span_target(self, span_token_ids_list, event_arg_idxs_objs_list):
        span2span_target = np.zeros((len(span_token_ids_list), len(span_token_ids_list)), dtype=np.int) + 1e-10
        for event_idx, event_arg_idxs_objs in enumerate(event_arg_idxs_objs_list):
            if event_arg_idxs_objs is None:
                continue
            for event_arg_idxs in event_arg_idxs_objs:
                for i, span_idx1 in enumerate(event_arg_idxs):
                    if span_idx1 is None:
                        continue
                    for j, span_idx2 in enumerate(event_arg_idxs):
                        if span_idx2 is None:
                            continue
                        span2span_target[span_idx1, span_idx2] = 1
        return span2span_target

    def generate_graph_info_for(self, pred_span_token_tup_list, return_miss=False):
        ...
        pred_span2span_target = self.build_span2span_target(pred_span_token_tup_list, pred_event_arg_idxs_objs_list)

        if return_miss:
            return pred_dag_info, pred_span2span_target, missed_span_idx_list, missed_sent_idx_list
        else:
    ...
    def convert_example_to_feature(self, ex_idx, dee_example, log_flag=False):
        ...
        for sent_idx, sent_text in enumerate(dee_example.sentences):
            ...
            ner_example = NERExample(
                '{}-{}'.format(annguid, sent_idx), sent_text, srange_mspan_mtype_tuples, dee_example.cut_word_label[sent_idx]
            )
            ...
            doc_cut_word_label_mat.append(ner_feature.cut_word_label)
        ...
        for mspan in dee_example.ann_valid_mspans:
            ...
            span_label_list.append(self.ner_fea_converter.entity_label2index.get('B-' + dee_example.ann_mspan2guess_field[mspan], 0))
        ...
        dee_feature = DEEFeature(
            annguid, ex_idx, doc_token_id_mat, doc_token_mask_mat, doc_token_label_mat, doc_cut_word_label_mat,
            span_token_ids_list, span_dranges_list, span_label_list, event_type_labels, event_arg_idxs_objs_list,
...
def prepare_doc_batch_dict(doc_fea_list):
    doc_batch_keys = ['ex_idx', 'doc_token_ids', 'doc_token_masks', 'doc_token_labels', 'doc_cut_word_labels', 'valid_sent_num']
    doc_batch_dict = {}

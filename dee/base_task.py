...
class BasePytorchTask(object):
    ...
    def base_train(self, get_loss_func, kwargs_dict1={},
        ...
        for epoch_idx in trange(base_epoch_idx, int(self.setting.num_train_epochs), desc="Epoch"):
            ...
            if self.only_master_logging:
                if self.is_master_node():
                    pbar = tqdm(train_dataloader, desc=iter_desc)
            ...
            for step, batch in step_batch_iter:
                ...
                if self.setting.model_type == 'Graph':
                    ...
                    accum_span2span_loss += (sum(info['span2span_loss'] for info in record_info)) / len(record_info)
                    accum_total_loss += (sum(info['total_loss'] for info in record_info)) / len(record_info)

                    pbar.set_description('span2span_loss: %.3f, total_loss: %.3f' % (
                        accum_span2span_loss / (step + 1),
                        accum_total_loss / (step + 1)
                        )
                    )
    ...
    def base_eval(self, eval_dataset, get_info_on_batch, reduce_info_type='mean', dump_pkl_path=None, **func_kwargs):
        ...
        for step, batch in enumerate(tqdm(eval_dataloader, desc=iter_desc)):
            ...
            with torch.no_grad():
                ...
                batch_info, batch_ner_info = get_info_on_batch(self, batch, **func_kwargs)
            ...
            if isinstance(batch_info, torch.Tensor):
                ...
                total_ner_info.extend(batch_ner_info)
        ...
        return reduced_info, total_ner_info

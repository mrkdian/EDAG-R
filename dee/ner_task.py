...
class NERExample(object):
    ...
    def __init__(self, guid, text, entity_range_span_types, cut_word_label):
        self.guid = guid
        self.text = text
        self.num_chars = len(text)
        self.entity_range_span_types = sorted(entity_range_span_types, key=lambda x: x[0])
        self.cut_word_label = cut_word_label
...
class NERFeature(object):
    def __init__(self, input_ids, input_masks, segment_ids, label_ids, cut_word_label, seq_len=None):
...
class NERFeatureConverter(object):
    ...
    def convert_example_to_feature(self, ner_example, log_flag=False):
        ...
        ex_cut_word_label = ner_example.cut_word_label

        assert len(ex_tokens) == len(ex_entity_labels)
        assert len(ex_tokens) == len(ner_example.text)
        ...
        if len(ex_tokens) > valid_token_len:
            ...
            if ex_cut_word_label is not None:
                ex_cut_word_label = ex_cut_word_label[:valid_token_len]
        ...
        return NERFeature(fea_input_ids, fea_masks, fea_segment_ids, fea_label_ids, ex_cut_word_label, seq_len=fea_seq_len)

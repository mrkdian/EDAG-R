...
from .doc_ner import DocNERModel

class DEETaskSetting(TaskSetting):
    base_key_attrs = TaskSetting.base_key_attrs
    base_attr_default_pairs = [
        ('train_file_name', 'extra_sample_train.json'),
        ('dev_file_name', 'extra_dev.json'),
        ('test_file_name', 'extra_test.json'),
        ...
        # ('cpt_file_name', 'Graph'),  # decide the identity of checkpoints, evaluation results, etc.
        # ('model_type', 'Graph'),  # decide the model class used
...
class DEETask(BasePytorchTask):
    ...
    def __init__(self, dee_setting, load_train=True, load_dev=True, load_test=True,
        ...
        self.example_loader_func = DEEExampleLoader(self.setting.rearrange_sent, self.setting.max_sent_len, self.setting.load_ex_num)
        ...
        elif self.setting.doc_ner:
            ner_model = DocNERModel(self.setting)
        else:
            ner_model = None
        ...
        elif self.setting.model_type == 'Graph':
    ...
    def resume_save_eval_at(self, epoch, resume_cpt_flag=False, save_cpt_flag=True):
        ...
        if self.setting.model_type == 'DCFEE':
            eval_tasks = product(['dev'], [False, True], ['DCFEE-O'])
        elif self.setting.model_type == 'Doc2EDAG':
            if self.setting.add_greedy_dec:
                eval_tasks = product(['dev'], [False, True], ['GreedyDec'])
            else:
                eval_tasks = product(['dev'], [False, True], [None])
        elif self.setting.model_type == 'Graph':
            eval_tasks = product(['dev'], [False, True], [None])
        ...
        for task_idx, (data_type, gold_span_flag, heuristic_type) in enumerate(eval_tasks):
            ...
            ner_dump_name = 'dee_ner.{}.{}.{}.{}.json'.format(data_type, span_str, model_str, epoch)

            self.eval(features, dataset, use_gold_span=gold_span_flag, heuristic_type=heuristic_type,
                      dump_decode_pkl_name=decode_dump_name, dump_eval_json_name=eval_dump_name, dump_ner_json_name=ner_dump_name)
    ...
    def eval(self, features, dataset, use_gold_span=False, heuristic_type=None,
             dump_decode_pkl_name=None, dump_eval_json_name=None, dump_ner_json_name=None):
        ...
        total_event_decode_results, total_ner_results = self.base_eval(
        ...
        if dump_eval_json_name is not None:
            dump_ner_json_path = os.path.join(self.setting.output_dir, dump_ner_json_name)
            self.logging('Dumping ner results into {}'.format(dump_ner_json_name))
        else:
            dump_ner_json_path = None
        ...
        total_ner_eval_results = self.measure_ner_prediction(
            total_ner_results, features, dump_ner_json_path
        )

        return total_event_decode_results, total_eval_res

    def measure_ner_prediction(self, pred_results, features, dump_json_path):
        ner_res = {}
        entity_label2index = self.feature_converter_func.entity_label2index
        for label in self.entity_label_list[1:]:
            ner_res[label[2:]] = {
                'tp': 0,
                'fp': 0,
                'fn': 0
        ...
        for pred_res, fea in zip(pred_results, features):
            for i, gt_span in enumerate(fea.span_token_ids_list):
                gt_dranges = fea.span_dranges_list[i]
                gt_label = fea.span_label_list[i]
                if gt_label == 0:
                    continue

                pred_dranges = pred_res.get(gt_span)
                if pred_dranges is not None:
                    for gt_drange in gt_dranges:
                        pred_label = pred_dranges.get(gt_drange)
                        if pred_label == gt_label:
                            ner_res[self.entity_label_list[pred_label][2:]]['tp'] += 1
                            pred_res[gt_span].pop(gt_drange)
                        else:
                            if pred_label is not None:
                                ner_res[self.entity_label_list[pred_label][2:]]['fp'] += 1
                                pred_res[gt_span].pop(gt_drange)
                            ner_res[self.entity_label_list[gt_label][2:]]['fn'] += 1
                else:
                    for gt_drange in gt_dranges:
                        ner_res[self.entity_label_list[gt_label][2:]]['fn'] += 1
            
            for pred_span, pred_dranges in pred_res.items():
                for pred_drange, pred_label in pred_dranges.items():
                    ner_res[self.entity_label_list[pred_label][2:]]['fp'] += 1
        ...
        ner_res['macro_f1'] = total_f1 / len(self.entity_label_list[1:])
        
        default_dump_json(ner_res, dump_json_path)
        # json.dump(ner_res, open(dump_json_path, mode='w'), indent=2)
